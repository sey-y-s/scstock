...
//         let fractionText = '';
//         if (Math.abs(fraction - 0.25) < 0.01) fractionText = '1/4';
//         else if (Math.abs(fraction - 0.50) < 0.01) fractionText = '1/2';
...
export default function Edit({ auth, movement, existingItems, warehouses, suppliers, customers }) {
    const [items, setItems] = useState(movement.items);
    const [showNewSupplier, setShowNewSupplier] = useState(false);
    const [showNewCustomer, setShowNewCustomer] = useState(false);
    ...
    const getTotal = () =>
        items.reduce((sum, item) => sum + item.quantity * (item.unit_price || 0), 0);
                ...
                <div className="max-w-6xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white shadow-sm rounded-lg p-8">
                        ...
                        <form onSubmit={submit} className="space-y-8">
                            {/* INFOS DE BASE */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* Gestion dépôt / client / fournisseur (inchangé) */}
                                {movement.type === 'in' && (
                                    <>
                                        {/* DEPOT DESTINATION */}
                                        <div>
                            ...
                            {/* 🔍 SECTION PRODUITS (nouveau composant intégré) */}
                            <div className="border-t pt-8">
                                <h3 className="text-lg font-semibold mb-4">Produits du mouvement</h3>
                                <ProductSearchWithCart
                                    onCartUpdate={setItems}
                                    warehouseId={movement.type === 'out' ? data.from_warehouse_id : data.to_warehouse_id}
                            ...
                            <div className="border-t pt-8 flex justify-between items-center">
                                <div className="text-lg font-semibold text-gray-700">
                                    Total : <span className={`text-${typeConfig.color}-600`}>
                                ...
                                <div className="flex space-x-4">
                                    ...
                                    <button
                                        ...
                                        className={`px-8 py-2 text-white rounded-md hover:opacity-90 disabled:opacity-50 font-medium bg-${typeConfig.color}-500`}
                                    >
                                        {processing ? 'Mise à jour...' : 'Mettre à jour'}
                                    </button>
                                </div>
